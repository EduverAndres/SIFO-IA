...
// Backend NestJS está en puerto 3001 con prefijo /api/v1
...
export const getPucCuentas = async (filtros = {}) => {
  try {
    ...
    const url = `${API_BASE_URL}/puc${queryParams.toString() ? '?' + queryParams.toString() : ''}`;
...
export const getPucCuentaPorCodigo = async (codigo) => {
  try {
    const url = `${API_BASE_URL}/puc/codigo/${codigo}`;
...
export const crearCuentaPuc = async (cuentaData) => {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/puc`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Accept': 'application/json',
...
export const eliminarCuentaPuc = async (id) => {
  try {
    ...
    const response = await authenticatedFetch(`/puc/${id}`, {
      method: 'DELETE'
    });
